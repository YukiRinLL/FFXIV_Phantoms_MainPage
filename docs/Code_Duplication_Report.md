...
> **状态校正（2026-09-03）**：本文原始内容混合了历史扫描结果、重构计划和“已完成”结论。当前源码只能确认部分共享配置已被引用，不能据此确认表中所有 22/24 个文件已经完成去重。本文的“预期效果”和“执行步骤”是计划，不是验收结果；重新执行扫描后才能更新数量。
...
|----------|-------------|-------------|------|
| Supabase API 配置硬编码 | 24 | 22 | ⚠️ 历史结论，需复核 |
| `getCookie()` 函数 | 8 | 8 | ⚠️ 历史结论，需复核 |
| 后端 API URL 硬编码 | 15 | 12 | ⚠️ 历史结论，需复核 |
| `generateUUID()` 函数 | 2 | 2 | ⚠️ 历史结论，需复核 |
| `cacheImage()` / `formatFileSize()` | 5 | 5 | ⚠️ 历史结论，需复核 |
| jQuery `.load()` 菜单加载模式 | 30+ | - | ⏳ 待后续处理 |
| `tools/` 下 HTML 模板克隆 | 3 | - | ⏳ 待后续处理 |
| Kook/Oopz 页面克隆 | 2 | - | ⏳ 待后续处理 |

**历史报告声称的剩余硬编码 API Key 位置**（当前仍需安全复核）：
- `assets/js/config.js` — 配置中心（唯一真实来源）
- `lib/discussbase/.env` — 环境变量文件
- `.github/workflows/keep-supabase-active.yml` — CI/CD 脚本

当前源码仍能看到多个页面通过 `window.APP_CONFIG` 读取 Supabase 配置，同时保留了 URL fallback；因此不能把“配置中心”理解为已经完全消除了所有重复或暴露风险。
...
**后续建议：** 重新执行一次排除 `lib/` 第三方目录的静态扫描，逐项核实配置中心、API URL、工具函数和历史备份文件，再单独记录已验证的结果。
