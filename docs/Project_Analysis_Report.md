...
> **状态校正（2026-09-03）**：这是一份历史静态审计报告，不是当前安全验收报告。报告中的数量、文件行号、绝对 `file:///d:/...` 链接和“建议修复”状态都需要重新扫描确认；其中部分问题可能已经被后续改动缓解，部分问题仍可能存在。不要仅依据本报告判断当前风险。
...
- 旧主页仍包含大量静态 HTML、测试页、备份页和第三方资源；报告中的“业务源文件”数量不应理解为整个目录的文件总数。
- Supabase 和后端地址已经部分集中到 `assets/js/config.js`，但多个页面仍保留 fallback URL，不能据此认定配置已完全统一。
- 新版 Vue 前端已经存在于同级目录 `ffxiv_phantoms_mainpage_vue`，因此报告中“考虑使用 Vue + Java 重构”的表述应视为已开始实施，而不是未来计划。
- 如需重新生成本报告，应使用当前路径、当前行号，并把发现项、已修复项、待复现项分开记录。

---

**历史报告结论：** 曾审计约 36 个业务源文件，记录 45+ 个代码问题和 18+ 个安全问题。该结论未经过 2026-09-03 的重新验证，不应作为当前精确统计。
